...
from deploy.easydeploy import model as EM
...
def parse_args():
    ...
    parser.add_argument('--config', default="F:/Github/YOLO-World/configs/pretrain/yolo_world_v2_x_1280.py", help='test config file path')
    parser.add_argument('--checkpoint', default="F:/Github/YOLO-World/weights/yolo_world_v2_x_obj365v1_goldg_cc3mlite_pretrain_1280ft-14996a36.pth", help='checkpoint file')
    parser.add_argument(
        '--work-dir',
...
def export_model(runner, text, max_num_boxes, score_thr, nms_thr):
    ...
    texts = [[t.strip() for t in text.split(',')] + [' ']]
    base_model.reparameterize(texts)  # 预推理文本标签
...
def demo(runner, args):
    with gr.Blocks(title="YOLO-World") as demo:
        ...
        demo.launch(server_name='127.0.0.1',
                    server_port=8080, share=True)  # port 80 does not work for me
