...
import os
import json
from sam.FoodSAM.tools.enhance_semantic_masks import predict_sam_label

def collect_masks(enhanced_mask, num_class=104):
    # 创建一个字典来存储每个类别的多个 mask
    masks, classes = [], []
    # 遍历每个类别，生成对应类别的 mask
    for label in range(num_class):
        # 使用 NumPy 的布尔索引选择当前类别的像素
        class_mask = (enhanced_mask == label).astype(np.uint8)
        # 如果这个类别的像素存在，则进行连通域分析
        if class_mask.sum() > 0:
            # 进行连通域分析
            num_labels, labels_im = cv2.connectedComponents(class_mask)
            # 遍历所有连通域（从1开始，因为0是背景）
            for i in range(1, num_labels):
                individual_mask = (labels_im == i).astype(np.uint8) * 255
...
def background2category(img_folder, thresholds = [0.5, 0.6],
    ...
    for threshold in thresholds:
        metadata_file = os.path.join(img_folder, metadata_path)
        ...
        methods = os.listdir(os.path.join(img_folder, method_dir))
        for method in methods:
            method_path = os.path.join(img_folder, method_dir, method)
            with open(method_path, 'r', encoding="utf-8") as f:
                boxs = json.load(f)
            backgrounds = os.listdir(os.path.join(img_folder, background_dir))
            for background in backgrounds:
                background_path = os.path.join(img_folder, background_dir, background)
                parts = []
                ...
                temp_dir = os.path.join(img_folder, save_dir,  method.split('.')[0], background.split('.')[0])
...
# 实例增强分割
def instance_segment(img_folder, category_txt, color_list_path, num_class=104, area_thr=0.01, ratio_thr=0.5, top_k=80,
    ...
    predict_sam_label(img_folder, category_txt, masks_path_name, sam_mask_label_file_name, pred_mask_file_name,
    ...
    instance_category_info_path = os.path.join(img_folder, sam_mask_label_file_dir,
                                               sam_mask_label_file_name)

    sam_mask_folder = img_folder
    csv_path = os.path.join(img_folder, metadata_path)
    img_path = os.path.join(img_folder, 'input.jpg')
    ...
    instance_category_info = sorted(instance_category_info, key=lambda x: float(x.split(',')[4]),
                                    reverse=True)
    instance_category_info = instance_category_info[:top_k]
    enhanced_mask = np.zeros((h, w, 3)) # 实例增强标签（区分个体）
    save_path = os.path.join(str(save_dir), str(instance_mask_name))
    for info in instance_category_info:
        idx, label, count_ratio, area = info.split(',')[0], int(info.split(',')[1]), float(
            info.split(',')[3]), float(info.split(',')[4])
        ...
        if label == 0:
            continue
        elif label < num_class:  # 控制mask颜色，区分实例
    ...
    visualization_save(enhanced_mask, save_path, boxes_np, info, color_list, color_list2, num_class)

    return (save_path, ), (boxes, masks, info)  # results



# 全景增强分割
def panoramic_segment(img_folder, category_txt, color_list_path, num_class=104,  area_thr=0.01, ratio_thr=0.5, top_k=80,
                  masks_path_name="sam_mask/masks.npy",
                  panoramic_mask_name='panoramic_vis.png',
                  method_dir='object_detection',
                  metadata_path = 'sam_metadata.csv',
                  new_label_save_dir = 'background2category',
                  method='od_UniDet',  # 增强模型
                  sam_mask_label_file_name='sam_mask_label.txt',
                  pred_mask_file_name='pred_mask.png',
                  sam_mask_label_file_dir='sam_mask_label'
                  ):
    ...
    predict_sam_label(img_folder, category_txt, masks_path_name, sam_mask_label_file_name,
                      pred_mask_file_name, sam_mask_label_file_dir)

    # 全景专属-背景转类别
    background2category(img_folder,
    ...
    category_info_path = os.path.join(img_folder, new_label_save_dir,
                                      method, sam_mask_label_file_dir, 'threshold-0.5.txt')
    ...
    for info in category_info:
        ...
        if label == 0:
            continue
        elif label < num_class: # 所谓enhance其实就是上色
            enhanced_mask[sam_mask] = random_color()
        else:
            enhanced_mask[sam_mask] = color_list[label]
        masks.append(sam_mask)
    ...
    with open(csv_path) as f:
        ...
        for row in rows:
            if row[0] in box_cats:
                ...
                boxes.append([float(row[2]), float(row[3]), float(row[4]), float(row[5]), area])
                info.append((label, name))
    boxes_np = np.array(boxes)   # 全景分割结果保存
    visualization_save(enhanced_mask, save_path, boxes_np, info, color_list, color_list2, num_class)
    return (save_path, ), (boxes, masks, info)  # results
