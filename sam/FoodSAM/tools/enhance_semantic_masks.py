...
# 实例分割的结果实际就是sam的结果
# sam结果中，每个mask具有像素级的类别语义
def calculate_single_image_masks_label(mask_file, pred_mask_file, category_list, sam_mask_label_file_name,
 ...
 mask_index, category_id, category_name, category_count, mask_count
    ...
    with open(mask_category_path, 'w') as f:
        ...
        for i in range(sam_mask_data.shape[0]):
            ...
            logger.info(
                f"{folder_path}/sam_mask/{i} assign label: [ {single_mask_category_label}, {category_list[single_mask_category_label]}, {count_ratio:.2f}, {counts.sum() / shape_size:.4f} ]")
            f.write(
                f"{i},{single_mask_category_label},{category_list[single_mask_category_label]},{count_ratio:.2f},{counts.sum() / shape_size:.4f}\n")

    f.close()

# sam结果的像素语义导出(单例)
def predict_sam_label(img_folder, category_txt,
    ...
    mask_file_path = os.path.join(img_folder, masks_path_name)
    pred_mask_file_path = os.path.join(img_folder, pred_mask_file_name)
    if os.path.exists(mask_file_path) and os.path.exists(pred_mask_file_path):
        calculate_single_image_masks_label(mask_file_path, pred_mask_file_path, category_list,
...
def collect_masks(enhanced_mask, num_class=104):
    ...
    masks, classes = [], []
    shape = enhanced_mask.shape[:2]
    # 遍历每个类别，生成对应类别的 mask
    for label in range(num_class):
        # 使用 NumPy 的布尔索引选择当前类别的像素
        mask = np.zeros(shape, dtype=np.uint8)
        mask[enhanced_mask == label] = 255
        if np.any(mask == 255):
            masks.append(mask)
...
def enhance_masks(img_folder, category_txt, color_list_path, num_class=104,
                  area_thr=0, ratio_thr=0.5, top_k=80,
                  masks_path_name="sam_mask/masks.npy",
                  ...
                  sam_mask_label_file_dir='sam_mask_label'):
    predict_sam_label(img_folder, category_txt, masks_path_name, new_mask_label_file_name, pred_mask_file_name,
                      sam_mask_label_file_dir)
    color_list = np.load(color_list_path)
    color_list[0] = [238, 239, 20]
    category_info_path = os.path.join(img_folder, sam_mask_label_file_dir, new_mask_label_file_name)
    sam_mask_folder = os.path.join(img_folder)
    pred_mask_path = os.path.join(img_folder, pred_mask_file_name)
    ...
    sam_masks = np.load(os.path.join(str(sam_mask_folder), str(masks_path_name)))
    for info in category_info:
    ...
    return (vis_save_path, save_path), collect_masks(enhanced_mask, num_class)
